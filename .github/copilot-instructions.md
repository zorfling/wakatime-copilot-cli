...
CLI/npm package that integrates [WakaTime](https://wakatime.com) time-tracking into [GitHub Copilot CLI](https://docs.github.com/en/copilot/how-tos/copilot-cli) via session lifecycle hooks. Heartbeats are sent under the **"ai coding"** category and attributed to real files (not virtual placeholders).

## Commands

```sh
npm run build      # tsc — compiles src/ → dist/
npm run dev        # tsx src/cli.ts — run from source without building
npm test           # node --test --import tsx test/**/*.test.ts

# Run any subcommand from source:
npm run dev -- setup
npm run dev -- status
npm run dev -- hook sessionStart --debug
```

`dist/cli.js` is the published `bin` entry. `dist/` is gitignored; `prepublishOnly` enforces a build.

**Tests:** `npm test` is wired up but no `test/` directory exists yet. Manually verify hooks by piping a JSON payload with `--debug`:
...
ESM TypeScript (`"type": "module"`, `Node16` module resolution). **All intra-package imports use `.js` extensions** even though sources are `.ts` — required by Node16 resolution. Preserve this when adding imports.
...
### Hook flow (the core of the package)

[src/hook.ts](../src/hook.ts) `runHook()` is invoked by Copilot CLI on `sessionStart`, `postToolUse`, and `sessionEnd`. It:

1. Reads JSON from **stdin** (`{ cwd, sessionId, toolName?, toolOutput?, toolInput? }`) via [src/util.ts](../src/util.ts) `readStdin()`.
2. Skips self-recursion (tool names starting with `wakatime`).
3. Resolves the git repo root → project name (`getRepoRoot`).
4. **Rate-limits to 1 heartbeat / 60s per project** via a timestamp file in the state dir (`shouldRateLimit`). Main guard against heartbeat spam.
5. Ensures `wakatime-cli` is installed ([src/wakatime-cli.ts](../src/wakatime-cli.ts)).
6. Picks an **entity** (tracked file): first a path extracted from `toolInput` keys, else the most-recently-modified source file under cwd within the last 10 min (`findBestEntity`), else falls back to `--entity-type app` "GitHub Copilot CLI".
...
| [src/install.ts](../src/install.ts) | Writes hook config JSON (global `~/.copilot/hooks` or `--local` `.github/hooks`) |
